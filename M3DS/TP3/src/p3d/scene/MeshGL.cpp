...
void MeshGL::initBuffer() {
  ...
  for(unsigned int i=0;i<_mesh->nbFace();++i) {
    for(unsigned int j=0;j<_mesh->nbVertex(i);++j) {
      allAttrib[iV*8+0]=_mesh->position(i,j).x();
      allAttrib[iV*8+1]=_mesh->position(i,j).y();
      allAttrib[iV*8+2]=_mesh->position(i,j).z();

      allAttrib[iV*8+3]=_mesh->normal(i,j).x();
      allAttrib[iV*8+4]=_mesh->normal(i,j).y();
      allAttrib[iV*8+5]=_mesh->normal(i,j).z();

      allAttrib[iV*8+6]=_mesh->texCoord(i,j).x();
      allAttrib[iV*8+7]=_mesh->texCoord(i,j).y();
      ++iV;
    }
  }
...
void MeshGL::drawNormal() {
    vector<Vector3> pts;
    for(unsigned int i=0;i<_mesh->nbFace();++i) {
        for(unsigned int j=0;j<_mesh->nbVertex(i);++j) {
            pts.push_back(_mesh->position(i,j));
            pts.push_back(_mesh->position(i,j)+_mesh->normal(i,j));
        }
    }
