...
namespace p3d {
  ...
  typedef enum {Coordinate_Parent,Coordinate_Local,Coordinate_World} ECoordinate;
  ...
  class Object3D {
    ...
    Matrix4 parentLocal() const;
    Matrix4 localParent() const;

    void compose(const Object3D &f);

    void translate(const Vector3 &tr,ECoordinate f=Coordinate_Parent);
    void translate(double x,double y,double z,ECoordinate f=Coordinate_Parent) {translate(Vector3(x,y,z),f);} // TODO : should be the right way... (Vector3 => x,y,z)
    ...
    // returns the point/direction in *LOCAL* (f=LOCAL_COORDINATE and then u supposed to be given in PARENT) or in PARENT (f=PARENT_COORDINATE and then u supposed to be given in LOCAL)
    Vector3 pointTo(ECoordinate f,const Vector3 &u);
    void pointTo(ECoordinate f,Vector3 *u);
    Vector3 directionTo(ECoordinate f,const Vector3 &u);
    void directionTo(ECoordinate f,Vector3 *u);

    //! get the direction (i.e. the z axis) in the *PARENT* coordinate system
    p3d::Vector3 direction() const;
    //! set the direction (i.e. the z axis) with u expressed in the *PARENT* coordinate system. Caution : you may want use lookAt instead.
    void direction(const Vector3 &u);

    //! set the orientation such that the z direction point to pos (pos is expressed in *PARENT*).
    ...
    //! rotate this coordinate system around the *PARENT* axes (y then x). center is assumed to be in *PARENT* coordinates
