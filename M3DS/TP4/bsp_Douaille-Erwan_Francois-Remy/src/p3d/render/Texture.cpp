...
void Texture::toGL(const QImage &img) {
  _dataType=GL_UNSIGNED_BYTE;
  _dataFormat=GL_BGRA;
  init(img.width(),img.height(),0,  img.convertToFormat(QImage::Format_ARGB32).bits(),GL_RGBA);
}

void Texture::toSubGL(const QImage &img) {
  _dataType=GL_UNSIGNED_BYTE;
  _dataFormat=GL_BGRA;
  glTexSubImage2D(GL_TEXTURE_2D,0,0,0,img.width(),img.height(),_dataFormat, GL_UNSIGNED_BYTE, img.convertToFormat(QImage::Format_ARGB32).bits());
}
...
void Texture::setAlpha(const p3d::Color &c,double v,double radius) {
  if (_img!=NULL) {
...
void Texture::makeChecker(int w,int h,int nbLine,int nbColumn,const p3d::Color &c1,const p3d::Color &c2) {
