...
Matrix4 Object3D::parentLocal() const {
  Matrix4 result;
...
Matrix4 Object3D::localParent() const {
  Matrix4 result;
  result=parentLocal();
  result.invert();
  return result;
...
Vector3 Object3D::pointTo(ECoordinate f,const Vector3 &p) {
  Vector3 res(p);
  if (f==Coordinate_Local) {
	    res.sub(_position);
...
Vector3 Object3D::directionTo(ECoordinate f,const Vector3 &p) {
  Vector3 res(p);
  if (f==Coordinate_Local) {
	    res=conjugate(_orientation)*res;
	    res.mul(Vector3(1.0/_scale.x(),1.0/_scale.y(),1.0/_scale.z()));
...
void Object3D::pointTo(ECoordinate f,Vector3 *p) {
  if (f==Coordinate_Local) {
	    p->sub(_position);
...
void Object3D::directionTo(ECoordinate f,Vector3 *u) {
  if (f==Coordinate_Local) {
