...
public:
    ...
    friend Matrix4 p3d::operator-(const Matrix4 &m);
    /// returns m1*m2 (composition of m1 with m2)
    friend Matrix4 p3d::operator*(const Matrix4 &m1,const Matrix4 &m2);
    /// returns m1*p (transformation of p according to m1)
    friend Vector4 p3d::operator*(const Matrix4 &m,const Vector4 &p);
