...
public:
  ...
  Vector2 windowToNDC(const Vector2 &pointWindow) {return windowToNDC(pointWindow.x(),pointWindow.y());}
  Vector2 windowToNDC(double x,double y);
  /// sets *pointCam in camera coordinates from pointWindow given in pixels (returns false if pointWindow is out of the viewport)
  bool windowToCamera(const Vector2 &pointWindow, Vector3 *pointCam) {return windowToCamera(pointWindow.x(),pointWindow.y(),pointCam);}
  bool windowToCamera(double x,double y, Vector3 *pointCam);
  Vector3 windowToCamera(double x,double y);
