...
Vector3 Quaternion::interpolateDirection(const Vector3 &u,const Vector3 &v,double t) {
  ...
  double ilength;
//  t=1;
  ilength=(1-t)*u.length()+t*v.length();
  axis.normalize();
  double angle=u.angle(v);
  cout << "angle = " << angle << endl;
  angle=angle*t;
  Matrix4 q;
  q=Matrix4::fromAngleAxis(toDegree(angle),axis);

  Vector3 res=ilength*(q.transformDirection(uu));
  cout << "length = " << ilength << " ulength " << v.length() << endl;
...
void Quaternion::set(const Vector3 &ii,const Vector3 &jj,const Vector3 &kk) {
  ...
  *this=q*q2;
  ...
  /* // something s wrong (tried many formulas from internet; none is correct (i.e q*(1,0,0), q*(0,1,0), q*(0,0,1) dont give ii,jj,kk)
  Vector3 i=ii;
  Vector3 j=jj;
  Vector3 k=kk;
  i.normalize();
