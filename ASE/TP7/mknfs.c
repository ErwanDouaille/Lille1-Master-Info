...
unsigned int new_bloc(){
	...
	if(super.super_first_free==0)
		return 0;
	read_bloc_n(current_volume, super.super_first_free, &fb, sizeof(struct free_bloc_s));
	...
	if(fb.fb_nbbloc==1)	
		...
		fb.fb_nbbloc--;
		wirte_bloc_n(current_volume, super.super_first_free, &fb, sizeof(struct free_bloc_s));
	}
	return new;
}
...
void free_bloc(unsigned int bloc){
	...
	write_bloc_n(current_volume, bloc, &fb, sizeof(struct free_bloc_s));
}
